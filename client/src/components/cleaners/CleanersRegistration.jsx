...
export default function CleanersRegistration() {
  ...
  const [selectedCity, setSelectedCity] = useState('서울특별시'); // 시/도 선택 상태

  const [formData, setFormData] = useState({
    name: '이기사',
    gender: 'male',
    emailLocal: 'cleaner',
    emailDomain: 'icedoctor.demo',
    password: '12345678',
    passwordChk: '12345678',
    phonePrefix: '010',
    phoneMiddle: '9876',
    phoneLast: '5432',
    locations: [1], // 선택된 지역들 저장 (location ID 배열)
    provider: 'NONE',
    profile: '',
    locationId: '',
  });
  ...
  const memoizedOptions = useMemo(() => {
    const locationArray = (locations.rows && locations.rows.length) ? locations.rows : [{ id: 1, city: '서울특별시', district: '전체' }, { id: 2, city: '대구광역시', district: '중구' }];
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      setIsLoading(true);
      const accounts = JSON.parse(localStorage.getItem('ice-doctor-demo-accounts') || '{}');
      accounts[payload.email] = { ...payload, role: 'CLEANER', profile: '/icons/default-profile.png' };
      localStorage.setItem('ice-doctor-demo-accounts', JSON.stringify(accounts));
      alert("회원가입이 완료되었습니다.");
      navigate('/login');
    } catch (error) {
